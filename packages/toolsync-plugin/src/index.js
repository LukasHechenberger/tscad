...
`,
  ...
  async setupPackage(package_, { rootPackage }) {
    if (!package_.isRoot) {
      const { homepage } = package_.packageJson;

      // if (!package_.packageJson.private)
      //   package_.packageJson.homepage = `${rootPackage.packageJson.homepage}/docs/api/modules/${package_.packageJson.repository.directory.split('/').at(-1)}`;
